...
const handleIncomingMsg = async(logger, session, pdu) => {
  ...
  try {
    ...
    if (!app || !app.messaging_hook) {
      ...
      return;
      /*
      return session.send(pdu.response({
        command_status: smpp.ESME_RDELIVERYFAILURE,
    ...
    logger.error({err}, 'Error sending SMS to feature server');
    /*
