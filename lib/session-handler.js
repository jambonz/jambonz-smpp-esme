...
const processFullMessage = async(logger, session, pdu, message_sid, message) => {
  ...
  const {lookupAppByPhoneNumber, retrieveSet} = require('..').locals.dbHelpers;
  const {source_addr: from, destination_addr: to} = pdu;
  ...
  try {
    ...
    logger.error({err}, 'processFullMessage - Error');
  }
};
...
const onPdu = (logger, session, pdu) => {
  switch (pdu.command) {
    ...
    case 'deliver_sm':
      ...
      break;
    default:
      logger.info({pdu}, 'got unhandled pdu');
      session.send(pdu.response());
      break;
  }
};
